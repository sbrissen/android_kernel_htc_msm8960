				 ...
				 enum as0260_width width)
{
	...
	case BYTE_LEN:{
			...
			buf[0] = waddr;
			buf[1] = wdata;
			rc = as0260_i2c_txdata(saddr, buf, 2);
		}
		break;
...
#if 0
static int as0260_i2c_write_table(struct as0260_i2c_reg_conf
				       *reg_conf_tbl, int num_of_items_in_table)
{
	...
	for (i = 0; i < num_of_items_in_table; i++) {
		rc = as0260_i2c_write(as0260_client->addr,
...
static int as0260_i2c_rxdata(unsigned short saddr,
			      unsigned char *rxdata, int length)
{
	...
	if (i2c_transfer_retry(as0260_client->adapter, msgs, 2) < 0) {
		pr_err("as0260_i2c_rxdata failed!\n");
		return -EIO;
	}
...
static int32_t as0260_i2c_read_w(unsigned short saddr, unsigned short raddr,
	unsigned short *rdata)
{
	...
	rc = as0260_i2c_rxdata(saddr, buf, 2);
	if (rc < 0)
		return rc;
	...
	*rdata = buf[0] << 8 | buf[1];
	...
	if (rc < 0)
		CDBG("as0260_i2c_read_w failed!\n");

	return rc;
}

#if 0
static int as0260_i2c_read(unsigned short saddr,
				unsigned short raddr, unsigned char *rdata)
{
	...
	memset(buf, 0, sizeof(buf));
	buf[0] = raddr;
	rc = as0260_i2c_rxdata(saddr, buf, 1);
	if (rc < 0)
		return rc;
	*rdata = buf[0];
	if (rc < 0)
		pr_info("as0260_i2c_read failed!\n");

	return rc;
}
#endif
static int as0260_i2c_write_bit(unsigned short saddr, unsigned short raddr,
unsigned short bit, unsigned short state)
{
	...
	pr_info("as0260_i2c_write_bit check_bit:0x%4x", check_bit);
	rc = as0260_i2c_read_w(saddr, raddr, &check_value);
	if (rc < 0)
	  return rc;

	pr_info("%s: as0260: 0x%4x reg value = 0x%4x\n", __func__,
		raddr, check_value);
	if (state)
	...
	pr_info("%s: as0260: Set to 0x%4x reg value = 0x%4x\n", __func__,
		raddr, check_value);

	rc = as0260_i2c_write(saddr, raddr, check_value,
		WORD_LEN);
	return rc;
}

static int as0260_i2c_check_bit(unsigned short saddr, unsigned short raddr,
unsigned short bit, int check_state)
{
	...
	for (k = 0; k < CHECK_STATE_TIME; k++) {
		as0260_i2c_read_w(as0260_client->addr,
			      raddr, &check_value);
		if (check_state) {
...
#if 0	
	if (as0260_s_ctrl.sensor_i2c_client && as0260_s_ctrl.sensor_i2c_client->client)
		as0260_client = as0260_s_ctrl.sensor_i2c_client->client;
	#if 1	
	as0260_i2c_check_bit(0, 0, 0, 0);
	as0260_i2c_write_bit(0, 0, 0, 0);
	#endif
#endif	
}
